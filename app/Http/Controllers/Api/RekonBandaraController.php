...
class RekonBandaraController extends Controller
{
    public function api_bandingkan_bandara($id)
    {
    ...
    public function api_error_maskapai($id)
    {
        // ===============================================================
        // Untuk Maskapai
        ...
        $data_a2 = $data_rekon_maskapai;
        $data_b2 = $data_rekon_admin;
    ...
    public function api_bandingkan_bandara_hapus(Request $request, $rekon_id)
    {
        ...
        if ($user_tipe == 'bandara') {
            $akun = Bandara::where('users_id', $user_id)->first();
        } else {
            $akun = BandaraStaf::where('users_id', $user_id)->first();
        }
        ...
        $data_rekon->update([
            'rekon_admin_text' => json_encode($data_rekon_admin_new, true)
        ]);
    ...
    public function api_bandingkan_bandara_tambah(Request $request, $id)
    {
    ...
    public function api_bandingkan_bandara_edit(Request $request, $id)
    {
        $data_rekon = Rekon::find($id);
        $baris_id = $request->baris_id;
        $user_id = $request->user_id;
        ...
        foreach ($data_rekon_admin as $items => $item) {
            if ($items == $baris_id) {
                $data_rekon_admin_new[$items] = $form_edit;
