...
const BandaraRekonBandingkan = () => {
    ...
    useEffect(() => {
        if (data_rekon.rekon_admin_text !== null) {
            ...
            setData_admin_2(JSON.parse(data_rekon.rekon_admin_text));
        }
                                ...
                                <table className='table' id='example'>
                                    ...
                                    <tbody>
                                        {data_maskapai.map((row, index) => (
                                            (JSON.stringify(row) !== JSON.stringify(data_admin[index]) && (
                                                < tr key={index} >
                                                    < td style={{
                                                        position: 'sticky',
                                                        left: 0,
                                                    ...
                                                    {
                                                        Object.values(row).map((value, i) => {
                                                            if (value === (Object.values(data_admin[index])[i])) {
                                                                return (
                                                                    <td key={i}>{value}</td>
                                                                );
                                                            } else {
                                                                return (
                                                                    <td style={{ color: 'red', flexDirection: 'column', flex: 1, }} key={i}>
                                                                        <div className="d-flex">
                                                                            <div className='text-primary'>{Object.values(data_admin[index])[i]}</div>
                                                                            <div className='text-muted'>&nbsp;-&nbsp;</div>
                                                                            <div className='text-danger'>{value}</div>
                                                                        </div>
                                                                        <input type="text" value={Object.values(data_admin_2[index])[i]} onChange={(e) => gantiData(index, i, e.target.value)} size={8} required />
                                                                    </td>
                                                                );
                                                            }
                                                        })
...
export default BandaraRekonBandingkan

if (document.getElementById('bandara_rekon_bandingkan')) {
    const domNode = document.getElementById('bandara_rekon_bandingkan');
    const root = createRoot(domNode);
    root.render(<BandaraRekonBandingkan data_rekon={window.data_rekon} data_maskapai={window.data_maskapai} />);
}