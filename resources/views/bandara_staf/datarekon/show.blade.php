                ...
                <div class="card shadow-sm">
                    <div class="card-body table-responsive">
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('bandara_staf.datarekon.create', $data_maskapai->id) }}"
                                class="btn btn-primary btn-sm mb-2">Tambah</a>
                            <a href="{{ route('bandara_staf.datarekon') }}"
                            ...
                            <tbody>
                                @foreach ($data_rekon as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{ route('bandara_staf.datarekon.destroy', $item->id) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
                                                <a href="{{ route('bandara_staf.datarekon.bandingkan', $item->id) }}"
                                                ...
                                                @else
                                                    @if ($item->riwayat_rekon->count() == 0)
                                                        @if (!$item->admin_status and !$item->maskapai_status)
                                                            <a href="{{ route('bandara_staf.datarekon.edit', $item->id) }}"
