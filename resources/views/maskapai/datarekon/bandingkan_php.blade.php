        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="card">
                                    ...
                                    @if (!in_array($b_item['AWB'], $tampung_kunci))
                                        <tr class="text-info">
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                <form
                                                    action="{{ route('maskapai.datarekon.bandingkan_tambah', $data_rekon->id) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('put')
                                    ...
                                    @if ($a_validasi_awb_tidak_ada[$a_items] == 'tidak')
                                        <tr class="text-danger">
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                <form
                                                    action="{{ route('maskapai.datarekon.bandingkan_hapus', $data_rekon->id) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('put')
                    ...
                    <div>
                        <div class="my-2">
                            @if (!$data_rekon->maskapai_status)
                                @if ($jumlah_error_bandara == 0 and $jumlah_error_maskapai == 0)
                                    <form action="{{ route('maskapai.datarekon.kirim', $data_rekon->id) }}"
                                        method="post">
                                        @csrf
                                ...
                                @endif
                            @elseif($data_rekon->maskapai_status == 2)
                                @if (!$data_rekon->maskapai_acc)
                                    <form action="{{ route('maskapai.datarekon.persetujuan', $data_rekon->id) }}"
                                        method="post">
                                        @csrf
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('maskapai.datarekon.bandingkan_edit', $data_rekon->id) }}" method="post">
                    @csrf
                    @method('put')
